...
const HydraulicPress = () => {
  ...
  const handleStartExperiment = () => {
    ...
    setTimeout(() => {
      const newPosition = force1 / 2 > force2 / 8 ? 'left' : force1 / 2 < force2 / 8 ? 'right' : 'neutral';
      setPistonPosition(newPosition);

      setExperimentResult({
        pressure1: calculatePressure(force1, 2),
        pressure2: calculatePressure(force2, 8),
        direction: newPosition === 'left' ? 'kichik shpris tomonga' :
          newPosition === 'right' ? 'katta shpris tomonga' :
            'harakatlanmaydi'
      });
    }, 1000);
  };
  ...
  const handleSaveExperiment = async () => {
    ...
    try {
      ...
      link.href = image;
      link.download = 'gidravlik-press-tajribasi.png';
      link.click();
    } catch (error) {
    ...
    >
            ...
            <CardContent>
              ...
              <Box display="flex" gap={2} mt={4} flexDirection="column">
                ...
                <Button
                  ...
                  disabled={!isExperimentRunning || !experimentResult}
        ...
        <Grid item xs={12} md={8}>
          <Card elevation={5} sx={{ height: '100%', borderLeft: '4px solid #4caf50' }}>
            <CardContent>
              ...
              >
                ...
                <Box sx={{
                  ...
                  <Box sx={{
                    ...
                    transform: pistonPosition === 'left' ? 'translateY(-30px)' :
                      pistonPosition === 'right' ? 'translateY(30px)' : 'translateY(0)'
                  }} />
                ...
                <Box sx={{
                  ...
                  <Box sx={{
                    ...
                    zIndex: 2,
                    transition: 'transform 0.5s ease',
                    transform: pistonPosition === 'right' ? 'translateY(-30px)' :
                      pistonPosition === 'left' ? 'translateY(30px)' : 'translateY(0)'
                  }} />
                ...
                {isExperimentRunning && pistonPosition !== 'neutral' && (
                  <>
                    <Box sx={{
                      position: 'absolute',
                      bottom: 125,
                ...
                {/* Direction indicator */}
                {isExperimentRunning && experimentResult && (
                  <Box sx={{
                    position: 'absolute',
                    top: 20,
                    left: '50%',
                    ...
                    <Typography variant="body1" fontWeight="bold" color={
                      experimentResult.direction === 'harakatlanmaydi' ? 'primary' :
                        experimentResult.direction === 'kichik shpris tomonga' ? 'error' : 'success'
                    }>
                      {experimentResult.direction === 'harakatlanmaydi' ? '⚖️ Porshen harakatlanmaydi' :
                        experimentResult.direction === 'kichik shpris tomonga' ? '← Porshen kichik shpris tomonga' :
                          '→ Porshen katta shpris tomonga'}
                    </Typography>
                  </Box>
                )}
              ...
              {isExperimentRunning && experimentResult && (
                <Box mt={2} p={2} bgcolor={
                  experimentResult.direction === 'harakatlanmaydi' ? '#e3f2fd' :
                    experimentResult.direction === 'kichik shpris tomonga' ? '#ffebee' : '#e8f5e9'
                } borderRadius={1}>
                  <Typography variant="body1" color={
                    experimentResult.direction === 'harakatlanmaydi' ? 'primary' :
                      experimentResult.direction === 'kichik shpris tomonga' ? 'error' : 'success'
                  } fontWeight="bold">
                    {experimentResult.direction === 'harakatlanmaydi' ?
                      "⚖️ Bosimlar teng - porshen harakatlanmaydi" :
                      experimentResult.direction === 'kichik shpris tomonga' ?
      ...
      <Box mt={3}>
        <Button
          ...
          endIcon={showTheory ? <ArrowDownwardIcon /> : <ArrowUpwardIcon />}
          fullWidth
          size="large"
        >
